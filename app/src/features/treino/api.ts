...
export async function execucaoAberta(): Promise<ExecucaoAberta | null> {
  const { data, error } = await supabase
    .from('execucoes_treino')
    .select(
      'id, treino_id, data, created_at, hora_inicio, duracao_minutos, execucoes_exercicio(id, exercicio_id, carga_real, reps_reais, rpe), execucoes_pulados(exercicio_id)',
    )
...
/**
 * Atualiza os dados da sessão: data, horário real e duração (resolução 10.24).
 *
 * O horário independe do que o fluxograma planejou — planejar às 18h e treinar às
 * 11h são dois fatos, e este registra o segundo. A duração é informada e **não**
 * derivada dos timestamps: `created_at` e `finalizado_em` medem o tempo de
 * registro, não o de treino.
 *
 * `null` em qualquer campo limpa o valor.
 */
...
/**
 * Uma sessão específica, com séries e pulados — o editor do histórico.
 *
 * Mesma forma de `execucaoAberta` de propósito: é o mesmo diálogo que edita a
 * sessão em andamento e a já finalizada, então os dois precisam alimentar a mesma
 * estrutura. Um segundo editor divergiria do primeiro na primeira mudança.
 */
export async function execucaoPorId(
  id: string,
): Promise<ExecucaoAberta | null> {
  const { data, error } = await supabase
    .from('execucoes_treino')
    .select(
      'id, treino_id, data, created_at, hora_inicio, duracao_minutos, execucoes_exercicio(id, exercicio_id, carga_real, reps_reais, rpe), execucoes_pulados(exercicio_id)',
...
export async function listarSeries(
  ...
  let consulta = supabase
    ...
    .select(
      'id, execucao_treino_id, exercicio_id, carga_real, reps_reais, rpe, execucoes_treino!inner(data, treino_id, created_at, finalizado_em, hora_inicio, duracao_minutos), exercicios_treino!inner(exercicio_base_id, biblioteca_exercicios!inner(nome, grupo_muscular))',
    )
  if (de) consulta = consulta.gte('execucoes_treino.data', de)
  if (ate) consulta = consulta.lte('execucoes_treino.data', ate)
  ...
  return (data ?? []).map((linha) => ({
    ...
    execucao_duracao_minutos: linha.execucoes_treino.duracao_minutos,
    grupo_muscular:
      linha.exercicios_treino.biblioteca_exercicios.grupo_muscular,
    exercicio_nome: linha.exercicios_treino.biblioteca_exercicios.nome,
  }))
